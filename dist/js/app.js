...
// EXTERNAL MODULE: ./node_modules/range-slider-input/dist/rangeslider.nostyle.umd.min.js
var rangeslider_nostyle_umd_min = __webpack_require__(927);
...
_t.bind("[data-fancybox-no-swipe]", {
    dragToClose: false
});

/* Плавный скролл */
const lenis = new lenis_modern_r();
...
if (search_show) {
    const search_animation = gsapWithCSS.to(search, {
        display: 'flex',
        opacity: 1
    }).pause();
...
if (nav_parent.length > 0) {
    nav_parent.forEach(item => {
        let target = item.querySelector('[data-nav-parent-target]'),
            nav_animation = gsapWithCSS.to(target, {
                display: 'grid',
                opacity: 1
            }).pause();
...
if (range.length > 0) {
    range.forEach(item => {
        let range_from = item.closest('[data-range-wrap]').querySelector('[data-range-from]'),
            ...
            rangeSliderElement = rangeslider_nostyle_umd_min(item, {
...
if (filter_title.length > 0) {
    filter_title.forEach(item => {
        ...
        if (filter_content) {
            let filter_animation = gsapWithCSS.to(filter_content, {
...
if (spoiler.length > 0) {
    spoiler.forEach(item => {
        let spoiler_parent = item.closest('[data-spoiler-parent]'),
            ...
            spoiler_animation = gsapWithCSS.to(spoiler_content, {
...
/* Аккордион */
const ac = new accordion_min('.accordion-container', {
    duration: 400
});
...
if (range_single) {
    ...
    let rangeSliderElement = rangeslider_nostyle_umd_min(range_single, {
...
if ( document.querySelector('.product-page__content-wrap') ) {
    const sticky_sidebar_init = new sticky_sidebar('.product-page__content-wrap', {
        topSpacing: 120,
        bottomSpacing: 0,
        minWidth: 991,
